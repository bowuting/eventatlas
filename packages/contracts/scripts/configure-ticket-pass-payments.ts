...
async function main() {
  const ticketPassAddress = process.env.TICKET_PASS_ADDRESS;
  if (!ticketPassAddress) {
    throw new Error("TICKET_PASS_ADDRESS is required");
  }

  const [deployer] = await ethers.getSigners();
  console.log("Configuring TicketPass with account:", deployer.address);
  console.log("TicketPass address:", ticketPassAddress);

  const ticketPass = await ethers.getContractAt("TicketPass", ticketPassAddress);
