...
import type {
  AttendanceProofActivityItem,
  CheckinItem,
  CheckinStatus,
  EventDetailItem,
  EventItem,
  OrganizerRatingStatItem,
  OrganizerHistoricalRatingItem,
...
type OrganizerHistoricalRatingRow = {
  event_id: string;
  title: string;
  start_at: Date | string;
  end_at: Date | string;
...
type AttendanceProofActivityRow = {
  id: string;
  title: string;
  category: string;
  cover_url: string | null;
...
type RecommendationCandidateRow = {
  ...
  start_at: Date | string;
  end_at: Date | string;
  recent_order_count: string | number;
  confirmed_checkin_count: string | number;
};

type UserAttendanceHistoryRow = {
  event_id: string;
  category: string;
  tags: string[] | null;
  lat: string | number;
...
function mapAttendanceProofActivityRow(row: AttendanceProofActivityRow): AttendanceProofActivityItem {
  return {
...
function mapRecommendationCandidateRow(row: RecommendationCandidateRow): RecommendationCandidateItem {
  return {
    eventId: Number(row.event_id),
    organizerWallet: row.organizer_wallet,
    title: row.title,
    ...
    confirmedCheckinCount: Number(row.confirmed_checkin_count)
  };
}

function mapUserAttendanceHistoryRow(row: UserAttendanceHistoryRow): UserAttendanceHistoryItem {
  return {
    eventId: Number(row.event_id),
    category: row.category,
    tags: row.tags ?? [],
...
function mapOrganizerRatingStatRow(row: OrganizerRatingStatRow): OrganizerRatingStatItem {
  return {
    organizerWallet: row.organizer_wallet,
...
export async function initDatabase() {
  ...
  await pool.query("CREATE INDEX IF NOT EXISTS idx_auth_nonces_wallet ON auth_nonces(wallet);");
  await pool.query("CREATE INDEX IF NOT EXISTS idx_auth_nonces_expires_at ON auth_nonces(expires_at);");
...
export async function getEventDetailWithTickets(eventId: number): Promise<EventDetailItem | null> {
  const eventResult = await pool.query<EventRow>(
    `SELECT * FROM events WHERE id = $1`,
    [eventId]
  );
  ...
  const [ticketResult, profileResult, historyResult] = await Promise.all([
    pool.query<TicketRow>(
      `SELECT * FROM ticket_types WHERE event_id = $1 ORDER BY id ASC`,
      [eventId]
    ),
    pool.query<OrganizerProfileRow>(
      `SELECT * FROM organizer_profiles WHERE wallet = $1`,
      [event.organizerWallet]
    ),
    pool.query<OrganizerHistoricalRatingRow>(
      `
        SELECT
          e.id AS event_id,
          e.title,
        ...
        LEFT JOIN reviews r
          ON r.event_id = e.id
          AND r.onchain_status = 'confirmed'
        WHERE e.organizer_wallet = $1
          AND e.id <> $2
          AND e.end_at < $3
        ...
        LIMIT 6
      `,
      [event.organizerWallet, event.id, event.startAt]
    )
  ]);

  const organizerHistoryRatings: OrganizerHistoricalRatingItem[] = historyResult.rows.map((row) => ({
    eventId: Number(row.event_id),
    title: row.title,
    startAt: asIso(row.start_at),
    endAt: asIso(row.end_at),
  ...
  const hasOrganizerProfile = (profileResult.rowCount ?? 0) > 0;
  ...
  return {
    ...
    organizer: {
      ...
      name: hasOrganizerProfile ? profileResult.rows[0].name : undefined,
      logoUrl: hasOrganizerProfile ? profileResult.rows[0].logo_url : undefined
    },
    organizerHistoryRatings
  };
}
...
export async function createAuthNonce(input: {
  ...
  const result = await pool.query<AuthNonceRow>(
    `
      INSERT INTO auth_nonces (wallet, nonce, issued_at, expires_at)
      VALUES ($1, $2, $3, $4)
      RETURNING *
    `,
    [input.wallet, input.nonce, input.issuedAt, input.expiresAt]
  );
...
export async function consumeAuthNonce(wallet: string, nonce: string) {
  const result = await pool.query<AuthNonceRow>(
    `
      UPDATE auth_nonces
      SET used_at = NOW()
...
export async function listAttendanceProofActivitiesByWallet(userWallet: string) {
  const result = await pool.query<AttendanceProofActivityRow>(
    `
      SELECT
        e.id,
        e.title,
  ...
  return result.rows.map(mapAttendanceProofActivityRow);
}

export async function listAttendanceProofActivitiesByEventIds(eventIds: number[]) {
...
export async function listRecommendationCandidatesByWallet(userWallet: string, limit = 40) {
  const safeLimit = Math.max(1, Math.min(limit, 200));
  const result = await pool.query<RecommendationCandidateRow>(
    `
      WITH user_joined_events AS (
      ...
      order_stats AS (
        SELECT
          o.event_id,
          COUNT(*)::BIGINT AS recent_order_count
        FROM orders o
      ...
      checkin_stats AS (
        SELECT
          c.event_id,
          COUNT(*)::BIGINT AS confirmed_checkin_count
        FROM checkins c
      ...
      SELECT
        ...
        e.start_at,
        e.end_at,
        COALESCE(os.recent_order_count, 0)::BIGINT AS recent_order_count,
        COALESCE(cs.confirmed_checkin_count, 0)::BIGINT AS confirmed_checkin_count
      FROM events e
      LEFT JOIN user_joined_events uje ON uje.event_id = e.id
      LEFT JOIN order_stats os ON os.event_id = e.id
      LEFT JOIN checkin_stats cs ON cs.event_id = e.id
      WHERE e.end_at >= NOW()
        AND e.chain_status = 'synced'
        AND uje.event_id IS NULL
      ORDER BY e.start_at ASC, e.id DESC
      LIMIT $2
    `,
    [userWallet, safeLimit]
  );

  return result.rows.map(mapRecommendationCandidateRow);
}

export async function listUserAttendanceHistoryByWallet(userWallet: string, limit = 120) {
  const safeLimit = Math.max(1, Math.min(limit, 500));
  const result = await pool.query<UserAttendanceHistoryRow>(
    `
      SELECT
        e.id AS event_id,
        e.category,
  ...
  return result.rows.map(mapUserAttendanceHistoryRow);
}

export async function listOrganizerRatingStatsByWallets(wallets: string[]) {
  if (wallets.length === 0) {
    return [];
  }

  const result = await pool.query<OrganizerRatingStatRow>(
    `
      SELECT
        e.organizer_wallet,
        ROUND(AVG(r.rating)::NUMERIC, 2) AS average_rating,
      ...
      LEFT JOIN reviews r
        ON r.event_id = e.id
        AND r.onchain_status = 'confirmed'
      WHERE e.organizer_wallet = ANY($1::TEXT[])
      GROUP BY e.organizer_wallet
    `,
    [wallets]
  );

  return result.rows.map(mapOrganizerRatingStatRow);
}
